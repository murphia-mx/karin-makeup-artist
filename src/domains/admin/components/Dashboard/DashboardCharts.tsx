...
const ChartSkeleton = () => (
  <div className="bg-white rounded-[2rem] p-8 border border-[#EBDDE2] shadow-[0_4px_24px_rgba(0,0,0,0.02)] flex flex-col h-full">
    <div className="mb-8">
      <div className="w-48 h-5 bg-[#EBDDE2]/50 animate-pulse rounded mb-2" />
      <div className="w-64 h-3 bg-[#EBDDE2]/50 animate-pulse rounded" />
    </div>
    <div className="flex-1 flex items-end gap-2 px-4 opacity-30">
      {[40, 70, 45, 90, 60, 30].map((h, i) => (
        <div key={i} className="flex-1 bg-[#CF7F9B] rounded-t-sm animate-pulse" style={{ height: `${h}%` }} />
      ))}
    </div>
  </div>
);
...
const PremiumTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-white/95 backdrop-blur-md p-4 rounded-xl shadow-lg border border-[#EBDDE2] min-w-[140px] transform transition-all duration-200">
        <p className="text-xs text-[#765E68] mb-3 font-medium uppercase tracking-wider">{label}</p>
        <div className="space-y-2">
          {payload.map((entry: any, index: number) => (
            <div key={index} className="flex justify-between items-center gap-4">
              <span className="text-sm font-medium text-[#301C27] capitalize flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full" style={{ backgroundColor: entry.color }} />
                {entry.name === REVIEW_STATUS.APPROVED ? 'Aprobadas' : entry.name === REVIEW_STATUS.PENDING ? 'Por leer' : entry.name}
              </span>
              <span className="text-sm font-bold text-[#301C27]">{entry.value}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <AnimatePresence mode="wait">
        {isLoading ? (
          ...
          <motion.div 
            ...
            className="lg:col-span-2 h-full flex items-center justify-center bg-[#FAF7F7] rounded-[2rem] border border-[#EBDDE2] border-dashed shadow-sm p-8 text-center"
          >
            <EmptyState
              icon={<Sparkles className="w-8 h-8 text-[#CF7F9B]/40" />}
          ...
          <>
            ...
            <motion.div
              ...
              className="bg-white rounded-[2rem] p-8 border border-[#EBDDE2] shadow-[0_4px_24px_rgba(0,0,0,0.02)] flex flex-col h-full hover:border-[#CF7F9B]/30 transition-colors duration-300"
            >
              <div className="mb-8 shrink-0">
                <h3 className="text-lg font-medium text-[#301C27]">El ritmo de tus opiniones</h3>
                <p className="text-sm font-light text-[#765E68]">¿Están aumentando tus reseñas?</p>
              </div>
                  ...
                  <AreaChart data={charts.approvalTrend} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    <defs>
                      <linearGradient id="colorApproved" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="#CF7F9B" stopOpacity={0.4}/>
                        <stop offset="95%" stopColor="#CF7F9B" stopOpacity={0}/>
                      </linearGradient>
                      <linearGradient id="colorPending" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="#EBDDE2" stopOpacity={0.4}/>
                        <stop offset="95%" stopColor="#EBDDE2" stopOpacity={0}/>
                      </linearGradient>
                    </defs>
                    <XAxis 
                      dataKey="week" 
                      axisLine={false} 
                      tickLine={false} 
                      tick={{ fontSize: 11, fill: '#765E68', fontWeight: 500 }} 
                      dy={10} 
                    ...
                    <YAxis 
                      ...
                      dx={-10}
                    />
                    <Tooltip cursor={{ stroke: '#EBDDE2', strokeWidth: 1, strokeDasharray: '4 4' }} content={<PremiumTooltip />} />
                    <Area 
                      type="monotone" 
