...
export class LandingBuilder {
  static build(
    ...
    const fallbackSubtitle = 'Maquillaje profesional diseñado para resaltar tu esencia en cada momento especial.';
    const defaultCover = 'https://images.unsplash.com/photo-1522337660859-02fbefca4702?q=80&w=1600&auto=format&fit=crop';
    const activeServices = services.filter(s => s.active);
    const mainServiceCategory = activeServices.length > 0 ? activeServices[0].category.toLowerCase() : 'belleza';
    ...
    const footer = {
      ...
      social: {
        whatsapp: workspace.whatsapp ? `https://wa.me/${workspace.whatsapp}` : '',
        instagram: workspace.instagram_handle ? `https://instagram.com/${workspace.instagram_handle}` : '',
        facebook: workspace.facebook_url || '',
        tiktok: workspace.tiktok_handle ? `https://tiktok.com/@${workspace.tiktok_handle}` : '',
      },
      contact: {
        email: '', 
        address: workspace.address || 'Ubicación no configurada',
        hours: workspace.schedule && Object.keys(workspace.schedule).length > 0 
      ...
      copyrightText: landing.footer_credits || `© ${new Date().getFullYear()} ${workspace.business_name || 'Mi Negocio'}. Todos los derechos reservados.`,
    };
    ...
    const featuredReviews = reviews.filter(r => landing.featured_review_ids?.includes(r.id));

    const testimonialsModel = {
      eyebrow: 'CLIENTAS FELICES',
      title: 'Lo que cuentan',
      italicWord: 'de su experiencia.',
      featuredReviews: featuredReviews.length > 0 ? featuredReviews : (reviews.length > 0 ? [reviews[0]] : []),
